...
export default function DashboardPage() {
  ...
  useEffect(() => {
    const unsubscribe = auth.onAuthStateChanged(async (user) => {
      ...
      if (user) {
        setLoading(true);
        try {
          // Fetch all invoices for stats. In a larger app, this might be aggregated on the backend.
          const allInvoicesQuery = query(collection(db, `users/${user.uid}/invoices`), orderBy("issueDate", "desc"));
  ...
  if (!currentUser || (invoices.length === 0 && customers.length === 0)) {
     return (
