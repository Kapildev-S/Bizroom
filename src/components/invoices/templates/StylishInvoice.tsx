                ...
                <table className="w-full text-left" style={{ tableLayout: 'fixed' }}>
                    ...
                    <thead>
                        <tr>
                            ...
                            {settings?.invoiceSettings?.enableAdvancedInvoiceSystem && <th className="p-2 pb-4 font-semibold text-gray-700 border-b-2 text-right">MRP</th>}
                    ...
                    <tbody>
                        {invoice.items.map((item, index) => (
                            <tr key={index}>
                                ...
                                {settings?.invoiceSettings?.enableAdvancedInvoiceSystem && <td className="p-3 border-b text-right font-medium whitespace-nowrap">{`${currencySymbol}${(item.mrp || item.unitPrice).toFixed(2)}`}</td>}
